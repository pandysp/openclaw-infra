...
| Brave Search API key | (Optional) Enables web search via Brave | brave.com/search/api → API Keys |
...
pulumi config set braveApiKey --secret            # Brave Search API key
...
Configured via Pulumi secret `braveApiKey`. If not set, deployment proceeds without web search. Get a free API key (2,000 queries/mo) at [brave.com/search/api](https://brave.com/search/api).

```bash
cd pulumi
pulumi config set braveApiKey --secret   # From brave.com/search/api → API Keys
