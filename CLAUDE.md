...
- Phoenix runs on a **dedicated staging tailnet**, never the operator's. A Phoenix server runs an autonomous agent with real API credentials for an hour and is publicly backed up; on its own tailnet it cannot reach the operator's devices or production by construction. The tailnet's whole policy is [docs/staging-tailnet-policy.hujson](./docs/staging-tailnet-policy.hujson).
- The staging server joins with a key minted per run from the `TS_OAUTH_CLIENT_ID`/`TS_OAUTH_SECRET` client of that tailnet (single use, ephemeral, pre-authorized, `tag:server`, one hour). No stored auth key exists to expire.
- Staging tailnet setup (once): create a separate Tailscale account/tailnet; DNS → enable MagicDNS and HTTPS certificates; paste the policy above; Settings → OAuth clients → generate one with scopes `auth_keys` (write) and `devices:core` (write) and tags `tag:ci`, `tag:server`; store its ID and secret as the repository secrets `TS_OAUTH_CLIENT_ID` and `TS_OAUTH_SECRET`.
...
- Teardown deletes only the proven node's API device ID, then reads it back as absent. A run whose deploy failed before proving a node passes cleanup only if no device carries its run name. Unrelated tailnet changes do not invalidate cleanup. Missing ownership or failed cleanup keeps the staging checkpoint for investigation.
- Deploy retries `nbg1 → fsn1 → hel1` only on Hetzner `resource_unavailable` (placement rejected before any server exists); every other failure stops immediately.
