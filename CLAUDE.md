...
- The staging server joins the tailnet with a key minted per run from the `TS_OAUTH_CLIENT_ID`/`TS_OAUTH_SECRET` client (single use, ephemeral, pre-authorized, `tag:server`, one hour). No stored auth key exists to expire; the OAuth client must own `tag:server` and `tag:ci` with the `auth_keys` scope.
