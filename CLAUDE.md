...
OpenClaw supports multiple inference providers. Default is Claude.

### Deploy with Kimi instead of Claude
...
|----------|-------|----------|
| `claude` | `anthropic/claude-opus-4-5` | high |
| `kimi` | `kimi-coding/k2p5` | high |
...
```yaml
openclaw_model_primary: "anthropic/claude-sonnet-4"  # explicit override
```

Then re-provision: `./scripts/provision.sh --tags config`
...
| Claude setup token | Powers OpenClaw (Claude provider) | `claude setup-token` in terminal |
| Kimi API key | Powers OpenClaw (Kimi provider) | platform.moonshot.ai |
