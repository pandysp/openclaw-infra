...
#   - pnpm installed (this script builds the ob runtime project itself; an
#     existing global ob is still honoured, but is no longer required)
...
# Create or refresh the ob runtime project, and prove its native addon loads.
#
# Why a project rather than `pnpm add -g obsidian-headless`: a global install
# lets the PACKAGE choose the better-sqlite3 version, and that addon only works
# on one Node major — which one changed with ob 0.0.13 and will change again.
# Chasing it with a Node pin in the deployer breaks in one direction or the
# other on every bump. Here the lockfile owns the pin and there is no knob.
#
# pnpm-workspace.yaml is not optional: pnpm 10+ refuses to run install scripts
# unless the package is listed, and then installs the addon with NO compiled
# binary at all — ob dies at require() with MODULE_NOT_FOUND for every ABI,
# which reads like a version mismatch but is not.
#
# package.json is written only when missing, so re-running is reproducible from
# the lockfile. To move versions, pass OB_VERSION explicitly.
ensure_ob_runtime() {
    local dir="$OB_RUNTIME_DIR"

    command -v pnpm >/dev/null 2>&1 || {
        warn "pnpm not found — cannot build the ob runtime project at $dir"
        return 1
    }

    mkdir -p "$dir"
    if [ ! -f "$dir/package.json" ] || [ "$OB_VERSION" != "latest" ]; then
        log "writing ob runtime manifest (obsidian-headless@${OB_VERSION})"
        cat > "$dir/package.json" <<JSON
{
  "name": "ob-runtime",
  "private": true,
  "dependencies": { "obsidian-headless": "${OB_VERSION}" }
}
JSON
    fi
    cat > "$dir/pnpm-workspace.yaml" <<'YAML'
# Generated by setup-mac-workspaces.sh — do not hand-edit.
# better-sqlite3 ships a native addon; without this allowance pnpm installs it
# with no compiled binary and ob fails at require().
allowBuilds:
  better-sqlite3: true
YAML

    ( cd "$dir" && pnpm install --no-frozen-lockfile ) >/dev/null 2>&1 || {
        warn "pnpm install failed in $dir"
        return 1
    }

    local bin; bin="$(ob_runtime_bin)"
    [ -x "$bin" ] || { warn "ob runtime built but $bin is missing"; return 1; }

    # Fail loud here rather than crash-looping later: a built addon is the whole
    # point of the project, so verify one actually loaded.
    if ! find "$dir/node_modules" -name 'better_sqlite3.node' -print -quit 2>/dev/null | grep -q .; then
        warn "ob runtime at $dir has no compiled better-sqlite3 — daemons would crash-loop"
        return 1
    fi
    log "ob runtime ready: $("$bin" --version 2>/dev/null || echo '?') at $dir"
}

# Run ob. The runtime project needs no Node pin (its lockfile owns the addon);
# a legacy bare global install still does.
ob_run() {
    if ob_runtime_present || [ ! -d "$OB_NODE_BIN" ]; then
        "$OB_BIN" "$@"
    else
...
# Build/refresh the runtime project, then re-resolve — resolve_ob_bin prefers it.
# A failure here is not fatal: an existing global ob still works, so fall through
# to the historical resolution rather than blocking setup on the new mechanism.
ensure_ob_runtime || warn "continuing with any existing global ob install"
OB_BIN="$(resolve_ob_bin || true)"
...
if [ ! -x "$OB_BIN" ] && ! command -v ob &>/dev/null; then
    ...
    if [ -z "$OB_BIN" ]; then
        echo "ERROR: ob CLI not found and the runtime project could not be built."
        echo "       Ensure pnpm is installed, then re-run; or install manually with:"
        echo "         pnpm add -g --allow-build=better-sqlite3 obsidian-headless"
        exit 1
    fi
fi
